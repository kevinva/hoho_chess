...
class DQN:
    ...
    def update(self, transition_dict):
        ...
        next_action_values = self.q_net(next_states_tensor)
        ...
        next_action_values_formatted = torch.zeros((batch_size, ACTION_DIM)).to(self.device)
        for i in range(batch_size):
            legal_actions = next_legal_actions_list[i]
            for action in legal_actions:
                action_idx = ACTIONS_2_INDEX[action]
                next_action_values_formatted[i][action_idx] = next_action_values[i][action_idx]
        max_next_actions = next_action_values_formatted.max(1)[1].view(-1, 1)
        max_next_q_values = self.target_q_net(next_states_tensor).gather(1, max_next_actions)
...
class MiniMaxDQN:
    ...
    def update(self, transition_dict):
        ...
        # print(f"next_planes: {next_planes}, players: {players}, {transition_dict['next_states']}")
        next_states_tensor = torch.stack(next_planes, dim = 0).to(self.device)
